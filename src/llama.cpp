...
struct llm_build_context {
 ...
 struct ggml_tensor * wk_b_view = ggml_cont(ctx0, ggml_view_3d(ctx0, model.layers[il].wk_b, n_embd_head_qk_nope, kv_lora_rank, n_head,
...
q_nope_view = ggml_cont(ctx0, ggml_permute(ctx0, q_nope_view, 0, 2, 1, 3));
cb(q_nope_view, "q_nope_view_perm", il);

struct ggml_tensor * q_nope_absorbed = ggml_cont(ctx0, ggml_mul_mat(ctx0, wk_b_view, q_nope_view));
cb(q_nope_absorbed, "q_nope_absorbed", il);

q_mqa_view = ggml_cont(ctx0, ggml_permute(ctx0, q_mqa_view, 0, 2, 1, 3));
...
struct ggml_tensor * q_compressed_view = ggml_cont(ctx0, ggml_view_2d(ctx0, q_compressed, kv_lora_rank + n_embd_head_qk_rope, n_head * n_tokens,
        ggml_row_size(q_compressed->type, kv_lora_rank + n_embd_head_qk_rope),
        0));
cb(q_compressed_view, "q_compressed_view", il);

struct ggml_tensor * kv_compressed_view = ggml_cont(ctx0, ggml_view_3d(ctx0, kv_compressed, kv_lora_rank, 1, n_tokens,
        ggml_row_size(kv_compressed->type, kv_lora_rank),
        ggml_row_size(kv_compressed->type, kv_lora_rank),
        0));
...
struct ggml_tensor * k_compressed_view = ggml_cont(ctx0, ggml_view_2d(ctx0, k_compressed, kv_lora_rank + n_embd_head_qk_rope, n_tokens,
        ggml_row_size(k_compressed->type, kv_lora_rank + n_embd_head_qk_rope),
        0));
...
struct ggml_tensor * k_cache_slots_view = ggml_cont(ctx0, ggml_view_1d(ctx0, kv_self.k_l[il], n_tokens * (kv_lora_rank + n_embd_head_qk_rope),
        ggml_row_size(kv_self.k_l[il]->type, kv_lora_rank + n_embd_head_qk_rope) * kv_head));
cb(k_cache_slots_view, "k_cache_slots_view", il);

struct ggml_tensor * v_cache_trans_slots_view = ggml_cont(ctx0, ggml_view_2d(ctx0, kv_self.v_l[il], n_tokens, kv_lora_rank,
        ggml_row_size(kv_self.v_l[il]->type, kv_self.size),
...
struct ggml_tensor * k_cache_view = ggml_cont(ctx0, ggml_view_2d(ctx0, kv_self.k_l[il], kv_lora_rank + n_embd_head_qk_rope, n_kv,
        ggml_row_size(kv_self.k_l[il]->type, kv_lora_rank + n_embd_head_qk_rope),
        0));
cb(k_cache_view, "k_cache_view", il);

struct ggml_tensor * v_cache_trans_view = ggml_cont(ctx0, ggml_view_2d(ctx0, kv_self.v_l[il], n_kv, kv_lora_rank,
        ggml_row_size(kv_self.v_l[il]->type, kv_self.size),
        0));
...
struct ggml_tensor * kq_view = ggml_cont(ctx0, ggml_view_3d(ctx0, kq, n_kv, n_tokens, n_head,
        ggml_row_size(kq->type, n_kv),
        ggml_row_size(kq->type, n_kv * n_tokens),
        0));
cb(kq_view, "kq_view", il);

struct ggml_tensor * kq_soft_max = ggml_cont(ctx0, ggml_soft_max_ext(ctx0, kq_view, KQ_mask, kq_scale, hparams.f_max_alibi_bias));
cb(kq_soft_max, "kq_soft_max", il);

struct ggml_tensor * kq_soft_max_view = ggml_cont(ctx0, ggml_view_2d(ctx0, kq_soft_max, n_kv, n_head * n_tokens,
        ggml_row_size(kq_soft_max->type, n_kv),
        0));
cb(kq_soft_max_view, "kq_soft_max_view", il);

struct ggml_tensor * kqv_compressed = ggml_cont(ctx0, ggml_mul_mat(ctx0, v_cache_trans_view, kq_soft_max_view));
cb(kqv_compressed, "kqv_compressed,", il);

struct ggml_tensor * wv_b_view = ggml_cont(ctx0, ggml_view_3d(ctx0, model.layers[il].wv_b, kv_lora_rank, n_embd_head_v, n_head,
...
struct ggml_tensor * kqv_compressed_view = ggml_cont(ctx0, ggml_view_3d(ctx0, kqv_compressed, kv_lora_rank, n_tokens, n_head,
        ggml_row_size(kqv_compressed->type, kv_lora_rank),
        ggml_row_size(kqv_compressed->type, kv_lora_rank * n_tokens),
        0));
cb(kqv_compressed_view, "kqv_compressed_view", il);

struct ggml_tensor * kqv = ggml_cont(ctx0, ggml_mul_mat(ctx0, wv_b_view, kqv_compressed_view));
cb(kqv, "kqv", il);

kqv = ggml_cont(ctx0, ggml_permute(ctx0, kqv, 0, 2, 1, 3));
cb(kqv, "kqv_perm", il);

cur = ggml_cont_2d(ctx0, kqv, n_head * n_embd_head_v, n_tokens);
cb(cur, "kqv_cont", il);
